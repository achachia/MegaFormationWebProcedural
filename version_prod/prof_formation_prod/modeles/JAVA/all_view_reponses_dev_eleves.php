...
function list_eleves($keygen_exo) {
    global $cxn;
    $liste = array();
    $table = 'Corrige_devoirs_users';
    $champ1 = 'fk_code_eleve';
    $champ2 = 'fk_keygen_exo';
    try {

        $sql = " SELECT    DISTINCT  Membre.code_user,CONCAT(Membre.nom,'.',Membre.prenom) AS  identite_eleve,Membre.profil_img," . $table . " .date_created,
             " . $table . " .keygen_rep," . $table . ".code_fiddle," . $table . ".date_update," . $table . ".keygen_rep," . $table . ".fk_statut_code," . $table . ".comment_code," . $table . ".nbr_points 
              FROM  Membre," . $table . "   WHERE  Membre.code_user=" . $table . "." . $champ1 . "        AND   Membre.user_actif='1'   AND   " . $table . "." . $champ2 . "=:param1 ORDER BY " . $table . ".date_update ";
        $resultat = $cxn->prepare($sql);
        $resultat->bindParam(':param1', $keygen_exo);
        $resultat->execute();
        $i = 0;
        while ($enregistrement = $resultat->fetch()) {
            ...
            $liste[$i]['keygen_rep'] = $enregistrement['keygen_rep'];
             $liste[$i]['nbr_points'] = $enregistrement['nbr_points'];
