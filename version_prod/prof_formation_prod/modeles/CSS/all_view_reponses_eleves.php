...
function list_eleves($keygen_exo) {
    ...
    try {
        ...
        $sql = " SELECT    DISTINCT  Membre.code_user,CONCAT(Membre.nom,'.',Membre.prenom) AS  identite_eleve,Membre.profil_img,Corrige_exercices_eleves.date_created,"
                ...
                . " FROM  Membre,Corrige_exercices_eleves   WHERE  Membre.code_user=Corrige_exercices_eleves.code_eleve        AND   Membre.user_actif='1'   AND   Corrige_exercices_eleves.keygen_exo=:param1 ORDER BY Corrige_exercices_eleves.date_update ";
        $resultat = $cxn->prepare($sql);
        $resultat->bindParam(':param1', $keygen_exo);
        $resultat->execute();
        $i = 0;
