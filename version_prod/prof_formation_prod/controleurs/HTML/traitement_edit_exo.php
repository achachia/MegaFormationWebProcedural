...
if ($etat) {
    ...
    $url = url_espace_formateur . '/index.php?module=HTML&action=all_view_exercices&result=succees';
} else {
    $_SESSION ['traitement_form_exo']['tableau_serialise'] = serialize($_POST);
    $_SESSION ['traitement_form_exo']['list_erreur'] = $list_erreur;
    $url = url_espace_formateur . '/index.php?module=HTML&action=all_view_exercices&result=echec';
}
//var_dump($etat);
header("Location:  $url");
