...
if ($etat) {
    ...
    $id_difficulte = $_POST['id_vote_dev'];
    $code_eleve = $_SESSION ['membre']['code_eleve'];
    $keygen_vote = random(20);
    try {
        $sql = " SELECT  id_vote  FROM  List_post_vote_devoirs  WHERE  fk_code_eleve='" . $_SESSION ['membre']['code_eleve'] . "'   AND  fk_keygen_devoir='" . $keygen_dev . "'  ";
        $select = $cxn->query($sql);
        $nb = $select->rowCount();
        if ($nb <= 0) {
            try {
                $sql = " INSERT INTO    List_post_vote_devoirs (date_created,fk_code_eleve,fk_keygen_devoir,fk_difficulte,keygen_vote)  VALUES (:param1,:param2,:param3,:param4,:param5)";
