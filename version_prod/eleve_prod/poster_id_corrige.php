...
if ($etat) {
    try {
        $sql = " SELECT  id_corrige  FROM  Corrige_exercices_eleves  WHERE  code_eleve='" . $_SESSION ['membre']['code_eleve'] . "'   AND  keygen_exo='" . $keygen_exo . "'  ";
        $select = $cxn->query($sql);
        $nb = $select->rowCount();
        if ($nb <= 0) {
            try {
                $sql = " INSERT INTO  Corrige_exercices_eleves (keygen_rep,keygen_exo,code_eleve,code_fiddle,version_code,date_created,nom_code,id_mode_public,comment_code,labels_code)  VALUES (:param1,:param2,:param3,:param4,:param5,:param6,:param7,:param8,:param9,:param10)";
...
//                $val_parametres_entree[] = $keygen_rep;
...
//                $val_parametres_entree[] = $labels_code;
            ...
            try {
                ...
                $sql = "    SELECT Liaison_exo_theme_user.date_limite_depot_user_travail "
                        . " FROM    Exercices_themes,Liaison_exo_theme_user   "
                        . " WHERE   Exercices_themes.keygen_exo= Liaison_exo_theme_user.fk_exo "
                        . "  AND   Exercices_themes.keygen_exo=:param1"
                        . "  AND   (Liaison_exo_theme_user.fk_keygen_groupe=:param2  OR  Liaison_exo_theme_user.fk_code_user=:param3) ";


                $resultat = $cxn->prepare($sql);
                $resultat->bindParam(':param1', $keygen_exo);
                $resultat->bindParam(':param2', $_SESSION ['membre'] ['keygen_groupe']);
                $resultat->bindParam(':param3', $_SESSION ['membre']['code_eleve']);
                $resultat->execute();
...
//            /*             * ************Mettre a jour le code et son statut ***************************** */
            try {
                $sql = " UPDATE  Corrige_exercices_eleves  SET   ";
                if ($drapeau) {
                    $sql .= " code_fiddle=:param1, ";
                }
                $sql .="  date_update=:param2,version_code=:param3,nom_code=:param4,id_mode_public=:param7,comment_code=:param8,labels_code=:param9,fk_statut_code='1'   WHERE  code_eleve=:param5   AND  keygen_exo=:param6 ";
                $stmt = $cxn->prepare($sql);
                if ($drapeau) {
...
//                $sql = formatage_requette_sql($sql, $val_parametres_entree);
    ...
    try {
        ...
        $sql = " SELECT  DISTINCT Liste_mode_public.nom_mode,Corrige_exercices_eleves.nom_code,Corrige_exercices_eleves.code_fiddle,Corrige_exercices_eleves.version_code"
                ...
                . " FROM  Corrige_exercices_eleves,Exercices_themes,Liste_mode_public,Liste_statut_code  "
        ...
        $parametres_sql = array($_SESSION ['membre']['code_eleve'], $keygen_exo);
