...
if ($etat) {
    ...
    try {
        $sql = " SELECT  id_comment  FROM  List_comments_exo  WHERE  code_eleve='" . $_SESSION ['membre']['code_eleve'] . "'   AND  keygen_exo='" . $keygen_exo . "'  ";
        $select = $cxn->query($sql);
        $nb = $select->rowCount();
        if ($nb <= 0) {
            try {
                $sql = " INSERT INTO    List_comments_exo (date_created,code_eleve,keygen_exo,comment)  VALUES (:param1,:param2,:param3,:param4)";
