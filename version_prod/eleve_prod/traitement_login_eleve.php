...
if ($etat) {
    ...
    /*     * ************************* compte eleve ********************************* */
    try {
        $sql = " SELECT code_user,nom,prenom,last_connection,type_compte,profil_img,keygen_groupe,user_banni FROM Membre  WHERE email='" . $email . "'  AND  password='" . $pass_crypte . "' AND  user_actif='1' ";
        $select = $cxn->query($sql);
        $nb = $select->rowCount();
        if ($nb > 0) {
            $enregistrement = $select->fetch();
            $type_compte_eleve = $enregistrement ['type_compte'];
            $_SESSION ['membre'] = array();
            $_SESSION ['membre'] ['code_eleve'] = $enregistrement ['code_user'];
            $_SESSION ['membre'] ['keygen_groupe'] = $enregistrement ['keygen_groupe'];
            ...
            if ($type_compte_eleve == '2') {
                $url_redirection = url_espace_eleve_dev;
            } elseif ($type_compte_eleve == '1') {
                $url_redirection = url_espace_eleve_prod;
            }

            /*             * ******************** User banni ************************** */
            $_SESSION ['membre']['user_banni'] = FALSE;
            if ($enregistrement['user_banni'] == '1') {
                $_SESSION ['membre']['user_banni'] = TRUE;
            }
            ...
            try {
                $sql = "UPDATE Membre SET  last_connection='" . $date_connection . "'  WHERE    email='" . $email . "'  AND  password='" . $pass_crypte . "' ";
...
if (!$etat) {
    $lien = url_espace_eleve_prod . '/login.php?message_erreur=erreur';
}
