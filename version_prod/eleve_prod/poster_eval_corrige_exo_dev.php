...
if ($etat) {
    ...
    try {
        $sql = " SELECT  id_post  FROM  List_post_degre_comprehension_corrige_devoir  WHERE  fk_code_eleve='" . $_SESSION ['membre']['code_eleve'] . "'   AND  fk_keygen_exo='" . $keygen_exo . "'  ";
        $select = $cxn->query($sql);
        $nb = $select->rowCount();
        if ($nb <= 0) {
            try {
                $sql = " INSERT INTO   List_post_degre_comprehension_corrige_devoir(date_created,fk_code_eleve,fk_id_degre,fk_keygen_exo)  VALUES (:param1,:param2,:param3,:param4)";
